/**
 * Export a deterministic transfer fixture for Foundry integration tests.
 */
...
async function main() {
  ...
  const vkey = JSON.parse(
    fs.readFileSync(path.join(buildDir, "transfer_dev_vkey.json"), "utf8")
  );
  ...
  const outPath = path.join(outDir, "transfer_dev_fixture.json");
