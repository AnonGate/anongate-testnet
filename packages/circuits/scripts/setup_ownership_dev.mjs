/**
 * LOCAL trusted setup for ownership_dev (off-chain selective disclosure).
...
const r1cs = path.join(buildDir, "ownership_dev.r1cs");
const zkey0 = path.join(buildDir, "ownership_dev_0000.zkey");
const zkeyFinal = path.join(buildDir, "ownership_dev_final.zkey");
const vkeyPath = path.join(buildDir, "ownership_dev_vkey.json");

async function main() {
  if (!fs.existsSync(r1cs)) {
    throw new Error("missing ownership_dev.r1cs — run: npm run compile:ownership-dev");
  }
  ...
  await snarkjs.zKey.contribute(
    zkey0,
    zkeyFinal,
    "absolute-privacy-ownership-dev",
    "absolute-privacy-ownership-dev-entropy"
  );
