/**
 * Groth16 setup + Solidity verifier export for transfer_dev.
 */
...
const r1cs = path.join(buildDir, "transfer_dev.r1cs");
const zkey0 = path.join(buildDir, "transfer_dev_0000.zkey");
const zkeyFinal = path.join(buildDir, "transfer_dev_final.zkey");
...
const verifierOut = path.resolve(
  ...
  "src",
  "verifiers",
  "TransferDevVerifier.sol"
);

async function main() {
  if (!fs.existsSync(r1cs)) throw new Error("missing transfer_dev.r1cs - compile first");
  if (!fs.existsSync(ptau)) {
    throw new Error(
      "missing ptau file (need powersOfTau28_hez_final_15.ptau)"
    );
  }

  // Incomplete prior setup can leave a broken zkey without section headers.
  ...
  await snarkjs.zKey.contribute(
    ...
    "absolute-privacy-dev-entropy-transfer"
  );
  ...
  source = source.replace(/contract Groth16Verifier/, "contract TransferDevVerifier");
