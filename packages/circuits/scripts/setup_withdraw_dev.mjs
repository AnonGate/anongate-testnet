/**
 * Groth16 setup + Solidity verifier export for withdraw_dev.
 */
...
const r1cs = path.join(buildDir, "withdraw_dev.r1cs");
const zkey0 = path.join(buildDir, "withdraw_dev_0000.zkey");
const zkeyFinal = path.join(buildDir, "withdraw_dev_final.zkey");
...
const verifierOut = path.resolve(
  ...
  "src",
  "verifiers",
  "WithdrawDevVerifier.sol"
);

async function main() {
  if (!fs.existsSync(r1cs)) throw new Error("missing withdraw_dev.r1cs - compile first");
  ...
  // Rename generated contract for clarity if needed.
  let source = fs.readFileSync(verifierOut, "utf8");
  source = source.replace(/contract Groth16Verifier/, "contract WithdrawDevVerifier");
