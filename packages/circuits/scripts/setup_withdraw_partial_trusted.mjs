/**
 * LOCAL TRUSTED SETUP for depth-20 withdraw_partial.
...
const verifierOut = path.resolve(
  ...
  "src",
  "verifiers",
  "WithdrawPartialTrustedVerifier.sol"
);
...
async function main() {
  ...
  console.log("groth16 setup (withdraw_partial depth-20)...");
  await snarkjs.zKey.newZKey(r1cs, ptau, zkey0);
  await snarkjs.zKey.contribute(
    zkey0,
    zkeyFinal,
    "absolute-privacy-trusted-local",
    "absolute-privacy-trusted-local-withdraw-partial"
  );
  ...
  source = source.replace(/contract Groth16Verifier/, "contract WithdrawPartialTrustedVerifier");
