...
contract ShieldedPoolWithdrawIntegrationTest is Test {
    ...
    WithdrawDevVerifierAdapter internal withdrawAdapter;
    ...
    bytes32[2] internal commitments;
    bytes32[2] internal nullifiers;
    ...
    function setUp() public {
        string memory json = vm.readFile("test/fixtures/withdraw_dev_fixture.json");
        if (!json.keyExists(".circuitRevision")) {
            vm.skip(true, "withdraw dev fixture must be regenerated for circuit revision 2");
        }

        commitments[0] = bytes32(vm.parseUint(json.readString(".commitments[0]")));
        commitments[1] = bytes32(vm.parseUint(json.readString(".commitments[1]")));
        nullifiers[0] = bytes32(vm.parseUint(json.readString(".nullifiers[0]")));
        nullifiers[1] = bytes32(vm.parseUint(json.readString(".nullifiers[1]")));
        ...
        WithdrawDevVerifier withdrawVerifier = new WithdrawDevVerifier();
        withdrawAdapter = new WithdrawDevVerifierAdapter(withdrawVerifier);
    ...
    function testRealProofWithdrawAgainstPool() public {
        ...
        for (uint256 i = 0; i < 2; ++i) {
            ...
            depositCommitment[0] = commitments[i];
            vm.prank(alice);
        ...
        // submitter is not alice (deposit wallet), which is the privacy-relevant path
        ...
        assertTrue(pool.isNullifierSpent(nullifiers[1]));
